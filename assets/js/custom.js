...
function filtreleAtikBankasi() {
  ...
  if (searchInput.length > 0) {
    ...
    for (let i = 0; i < wasteDatabase.length; i++) {
      ...
      if (item.name.toLowerCase().includes(searchInput) || item.category.toLowerCase().includes(searchInput)) {
        ...
        resultsHtml += '<div class="result-card">' +
                         '<span class="res-points">' + item.points + '</span>' +
                         '<h4>' + item.name + '</h4>' +
                         '<p>' + item.desc + '</p>' +
                       '</div>';
      }
    ...
    if (foundAny) {
      ...
      resultsGrid.innerHTML = '<div class="result-card" style="grid-column: 1 / -1; text-align: center; border-color: var(--good-orange); max-width: 100%; width: 100%;">' +
